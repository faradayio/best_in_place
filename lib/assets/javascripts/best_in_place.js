...
BestInPlaceEditor.forms = {
    "input": {
        activateForm: function () {
            ...
            const textInputs = this.element.find("input[type='text']");
            if (textInputs.length) {
              textInputs[0].select();
              if (!this.okButton) {
                  textInputs.bind('blur', {editor: this}, BestInPlaceEditor.forms.input.inputBlurHandler);
              }
              textInputs.bind('keyup', {editor: this}, BestInPlaceEditor.forms.input.keyupHandler);
            }

            const dateInput = this.element.find("input[type='date']");
            if (dateInput.length) {
              dateInput[0].select();
              dateInput.bind('input', {editor: this}, (event)=>{
                debugger;
                this.element.find("input").attr('value', this.getValue())
                BestInPlaceEditor.forms.input.inputBlurHandler(event);
                // dateInput.attr('value', this.display_value);
              });
            }
