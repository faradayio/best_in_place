...
BestInPlaceEditor.prototype = {
            ...
            switch (this.formType) {
                ...
                case 'textarea':
                    if (this.display_raw) {
                        ...
                        var rawValue = this.element.attr('data-bip-value');
                        var parsedValue = this.element.data('bipValue');
                        if (typeof rawValue === 'undefined') {
                            to_display = '';
                        } else if (this.jsonMatcher.test(rawValue)) {
                            to_display = rawValue.replace(/&amp;/gi, '&');
                        } else if (typeof parsedValue === 'string') {
                            to_display = parsedValue.replace(/&amp;/gi, '&');
                        } else {
                            to_display = parsedValue;
                        }
                    }
                    break;
                case 'select':
    ...
    initOptions: function () {
        ...
        self.jsonMatcher = /^[[{]/
        var rawOriginalContent = self.element.attr('data-bip-original-content')
        var parsedOriginalContent = self.element.data("bipOriginalContent")
        if (typeof rawOriginalContent === 'string' && self.jsonMatcher.test(rawOriginalContent)) {
          self.original_content = rawOriginalContent || self.original_content;
        } else {
          self.original_content = parsedOriginalContent || self.original_content;
        }
    ...
    sanitizeValue: function (s) {
        'use strict';
        // seamus may 15, 2025: we are disabling sanitization because it breaks SQL (and json)
        // there is a risk that admin employees will enter malicious code, but it's low, and they could do so anyway using psql
        // in terms of non-admin users, this function does not protect us anyway against XSS
        // https://github.com/faradayio/security_artifacts/issues/93
        // taken from https://github.com/janl/mustache.js/blob/972fd2b27a036888acfcb60d6119317744fac7ee/mustache.js#L60-L75
        // var result = String(s);
        // for (var [char, replacement] of Object.entries({
        //     '<': '[',
        //     '>': ']',
        //     '(': '[',
        //     ')': ']',
        ...
        activateForm: function () {
            ...
            $.each(a, function(index, arr){
                ...
                var option_elt = jQuery(document.createElement('option'))
                    .val(key)
                    .html(BestInPlaceEditor.prototype.sanitizeValue(value));
